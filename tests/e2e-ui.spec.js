...
test.describe('Rekenkern dealvoorstel', () => {
  ...
  test('ZOPA trade-space: buckets sommeren exact op verkocht belang + earn-up + behouden belang', async ({ page }) => {
    ...
    const { b, z } = r;
    // Kerninvariant (sinds ChatGPT-review 31 aug 2026): totaal == verkocht belang + earn-up(verkoper-basis) + behouden belang
    expect(Math.round(z.totaal)).toBe(Math.round(b.verkochtBelangWaarde + z.earnUp + z.behoudenBelangWaarde));
    ...
    // voorwaardelijk = uitgestelde earn-out + verkoperslening + voorwaardelijke earn-up
    expect(Math.round(z.voorwaardelijk)).toBe(Math.round(b.earnOutUitgesteld + 200000 + z.earnUp));
    expect(z.earnUp).toBeGreaterThan(0);  // earn-up bestaat → "voorwaardelijk" mag nooit 0 zijn
