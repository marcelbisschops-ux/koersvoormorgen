...
- **24 augustus 2026 — zevende volledige heraudit (op Marcels verzoek "cijfer voor het platform?" → "draai nu maar").** Vijf onafhankelijke deelaudits (architectuur/backend/data/API/performance/onderhoudbaarheid; security + volledige cross-path-checklist; frontend/UX/toegankelijkheid; teststrategie/cloud/deployment/compliance; M&A-functionaliteit + een eigen onafhankelijke herberekening van de volledige waarderingsrekenkern, 83/83 checks groen). Volledig rapport als artifact: https://claude.ai/code/artifact/54efbe55-007c-46db-a725-2c08e7fe8e28. **Score: 64/100, van 69 (19 augustus) — bewust géén gemiddelde van de 15 deelscores, lager gewogen.**

  Financiële correctheid (84→91) en Security (63→74) stegen allebei aantoonbaar. De score daalde toch omdat twee OPERATIONELE regressies zijn gevonden die precies dezelfde vangnetten raken die bij eerdere audits (25 juli) ook al eens stil hadden gefaald en toen "gefixt, geverifieerd" werden afgesloten:
  1. **CI draait sinds 25 juli 2026 geen enkele stap meer, in beide repo's** — een YAML-expressiefout (`${{ secrets.STAGING_ADMIN_KEY != '' }}`) breekt de workflow vóór de eerste stap; 249 commits sindsdien nooit door `node --check`/`audit-consistentie.mjs`/`npm audit`/e2e-suite gehaald.
  2. **Dagelijkse D1-back-up staat al 3 dagen stil (sinds 21 augustus), zonder alert** — `scripts/backup.sh` onderdrukt de wrangler-exportoutput (`>/dev/null`), dus zelfs het eigen logbestand geeft geen diagnose; handmatige herhaling van hetzelfde commando lukte meteen (wijst op een launchd/keychain-omgevingsprobleem, geen Cloudflare-storing).

  Twee nieuwe P1-toegankelijkheidsgaten op `matching-platform.html` (de enige nieuwe publieke pagina, gedeeld met externe M&A-kantoren): koperkaarten niet met toetsenbord bedienbaar (kernfunctie onbereikbaar voor toetsenbord/schermlezer), en de al bij de 19-augustus-audit gevonden `--muted`-contrastbevinding (WCAG AA-fail) is nog steeds niet gefixt in `index.html`/`kantoorscan.html`/`hugo.html` — en nu ook gebakken in deze nieuwe pagina.

  **7 P2's:** matching-platform-wachtwoord buiten de bedoelde rate-limiter om te raden (`matchingAuthOk()` heeft zelf geen limiter, alleen de cosmetische `/login`-route wel — concreet misbruikscenario: koper-claim-DoS + valse fee-events); teaser/verkoopmemorandum missen de striktere AI-kostenlimiter-tier; race condition op de teaser/verkoopmemorandum-eerstegeneratie-fee (dubbele belasting mogelijk, `isDubbeleVerzending()` bestond al maar werd niet hergebruikt); het cascade-veiligheidsnet (`audit-consistentie.mjs` check 6) scant alleen `cloudflare-worker.js`, niet de 20 `worker/*.js`-modules waar nieuwe tabellen tegenwoordig het vaakst ontstaan; foreign keys staan op productie maar niet in de broncode (`initDB()`) — schema-as-code-drift, een verse omgeving zou ze missen; `matching-platform.html`-formuliervelden missen `<label>`; `marilyn.html`-modals (incl. het nieuwe €-Verkoop-modal) missen `aria-modal`/`role="dialog"`.

  **7 P3's + 6 P4's** — zie artifact voor volledige lijst; noemenswaardig: dode `marketing_prijs`-kolom in `worker/16-adviseur.js` (rest van het oude, inmiddels vervangen prijsmodel — bevestigd 0 frontend-consumenten), N+1-query in het nieuwe matching-overzicht, `/beveiliging`-route mist security-headers (aanname in `getCORS()` — "geen route retourneert html" — klopt niet meer sinds het veiligheidsdashboard), sectorspecifieke waarderingsmethodiek beperkt tot EBITDA-vs-omzet-onderscheid (mogelijk bewuste scope-keuze, aan Marcel voorgelegd in het artifact, niet zelf ingevuld).

  **Positief, expliciet herbevestigd (regressie zou hier zichtbaar worden):** delete-cascade blijft geconsolideerd (27 tabellen + R2, alle 3 purge-paden); N+1 in `/gebruikers/lijst` blijft gefixt; matching-toggle geen IDOR; `platform_fees` geen cross-adviseur-lek; sessiemechanisme sound; CORS-whitelist echt afgedwongen; geen SQL-injectie/hardcoded secrets gevonden; het nieuwe "Veiligheid"-dashboard in marilyn.html is bevestigd een échte, werkende implementatie (echte cron, echte alerting), geen schijnvertoning; 44/44 e2e-API-tests groen; alle eerdere waarderings-P1's (DCF FCFE/WACC, Goodwill-overwinstmethode, sector-multiple-type, synergie-NPV na belasting) blijven correct; groepsniveau-discipline (18 augustus-fix) houdt stand; AI overschrijft nooit zelf een waarderingsuitkomst; risicoraamwerk compliant met de gouden standaard.

  **Nog niet opgepakt binnen deze sessie — Marcel prioriteert** (zelfde discipline als eerdere rondes: audit eerst, fixen op verzoek). Zie sessie-geheugen `project_validatie_audit_25juli` en het artifact voor de volledige, geprioriteerde lijst.

- **5 september 2026 — begrensde routine-ronde (geautomatiseerde scheduled task, aanvraag `AV1788511737513KK4F` via de "Draai diepe audit nu"-knop, ingediend 4 sep 2026).** Geen volledige multidisciplinaire heraudit — de vaste, begrensde ronde (bestaande checks + gerichte diff-review sinds de vorige diepe audit) zoals vastgelegd in de scheduled-task-instructie na herhaald vastlopen van de volledige ronde in een onbeheerde sessie. Volledig rapport als artifact: https://claude.ai/code/artifact/2f9b1e77-bba2-47ac-ad50-5a03c2595862. **Score: 67/100, van 64 (24 augustus) — voorzichtige bijstelling, geen herberekening van de tien dimensies.**

  **Checks: 482/482 geslaagd, 0 gefaald** (3 bewust overgeslagen — AI/e-mail, kosten): `node --check` op worker + 20 backend-modules + 7 mna-modules; frontend-consistentie-audit 9/9; backend-consistentie-audit 5/5; policy-equivalentie 54/54; bewaartermijn-engine 16/16; e2e-API 44/44; cross-path-regressietests 72/72; acht losse rekenkern-validatiescripts (BATNA/walk-away, ZOPA, opbrengstbrug, ruilverhouding, bod-vergelijker, maatschap-waardering, grondslag, edge-cases, verkoper-bedragen) samen 282/282.

  **Diff-review sinds 24 augustus:** het zwaartepunt zit in een nieuwe onderhandelmodule bij het dealvoorstel — BATNA/walk-awayprijs, ZOPA/onderhandelruimte, ruilverhouding (aandelenruil) en een bod-vergelijker (Deal Value Matrix) — `mna/03-rekenkern-waardering.js` (+1038 regels) en `mna/04-begeleider-dashboard.js` (+463 regels). Alle vier volledig gedekt door de validatiescripts hierboven; statisch bevestigd dat BATNA/walk-away uitsluitend in de interne bijlage staat, nooit richting de koper (bestaande check 8). Daarnaast: Cliëntacceptatie/Wwft-bevestigingsveld vóór documentverzending, VOK teruggebracht naar één canonieke bron, matching-zelfregistratie met verplichte akkoordverklaring. Backend-diff (schema/auth) beperkt tot idempotente kolomtoevoegingen op bestaande tabellen — geen nieuwe tabel buiten de bestaande cascade-dekking.
  ...
  **Tweede vervolg, zelfde dag — score nogmaals gecorrigeerd naar 68, ditmaal wegens twee eigen verificatiefouten.** Marcel zei "ja" op het aanbod om de twee resterende P2's (matching-wachtwoord-rate-limiter, foreign-keys-niet-in-broncode) zelfstandig te fixen. Bij het daadwerkelijk willen doorvoeren van die fixes bleken **beide al opgelost te zijn** — de "nog open"-conclusie in de vorige versie berustte op twee eigen fouten, niet op de werkelijke staat van de code: (1) de grep naar `matchingAuthOk()` zocht op `function matchingAuthOk` terwijl de functie als `const ... = async (req) => {}` is gedefinieerd, dus vond niets en werd verkeerd geïnterpreteerd als "geen limiter"; (2) de `grep -c "FOREIGN KEY"`-uitvoer gaf letterlijk `cloudflare-worker.js:20` terug (20 treffers), maar is fout gelezen/becommentarieerd als "0 treffers". Beide zijn nu met de daadwerkelijke code als bewijs rechtgezet in `OPEN-BEVINDINGEN.md`: `matchingAuthOk()` deelt al sinds 24 augustus een 10/uur-ratelimiet met `/mna/matching/login`; `initDB()` bevat 20 `CREATE TABLE`-statements met `FOREIGN KEY ... ON DELETE CASCADE`, die alle 19 gemigreerde tabellen dekken (de twee destijds "ontbrekende" tabellenamen bleken vestigiaal, nooit gebouwd). Bijvangst van deze tweede controleronde: de marilyn-modals-telling (P2-11) was zelf te grof — niet 1 van ~4 maar 1 van minstens 7 modal-functies mist nog `aria-modal`, dus dát punt is eerder erger dan eerder gemeld. Netto: **10 gefixt, 3 gedeeltelijk, 7 nog open, 2 wacht-op-Marcel, 2 niet gecontroleerd.** Score **68/100**. Geen van beide P2's is dus alsnog gefixt in deze sessie — er was niets te fixen. **Les, ook voor mezelf vastgelegd:** "de grep geeft niets terug" bewijst een afwezigheid alleen als het patroon zelf klopt, en commando-uitvoer moet daadwerkelijk gelezen worden — een vooraf ingevuld verwacht antwoord in hetzelfde bash-commando (zoals "leeg hierboven = nog steeds 0 treffers") is een valkuil om zelf niet meer kritisch naar de echte uitvoer te kijken.

- **5 september 2026 — achtste volledige heraudit (op Marcels expliciete verzoek "nu" gedraaid, ondanks een aanbeveling om het uit te stellen naar een frissere sessie gezien de omvang van de dag ervoor).** Vijf onafhankelijke, parallelle deelaudits via aparte subagents (architectuur/backend/data/API/performance/onderhoudbaarheid; security + volledige cross-path-checklist; frontend/UX/toegankelijkheid; teststrategie/cloud/deployment/compliance; M&A-functionaliteit + een eigen onafhankelijke herberekening van de volledige waarderingsrekenkern — alle 5 kernmethoden PASS, plus 282/282 checks op de onderhandelmodule). Drie van de vijf subagents liepen tijdens uitvoering vast (stream-watchdog, geen crash) en zijn hervat. Volledig rapport als artifact: https://claude.ai/code/artifact/ba7699ae-3e02-4782-9ef3-52285402d76b. **Score: 58/100, van 64 (24 augustus) — bewust géén gemiddelde van de vijftien deelscores, lager gewogen.**

  Financiële correctheid (91→92) en M&A-functionaliteit (82→86) stegen allebei — de hoogste financiële-correctheidsscore die deze audit ooit heeft gegeven. De score daalde toch, om dezelfde reden als de 69→64-correctie op 24 augustus: twee NIEUWE kritieke bevindingen zijn precies het patroon van een vangnet dat zelf zonder alarm faalt, of een bugklasse die dit platform al eerder trof.

  **P1 — kritiek (2):**
  1. **`/mna/chat/{code}` — koper leest het vertrouwelijke verkoper↔begeleider-gesprek + kan zich voordoen als de begeleider.** GET/POST herleiden verkoper-, koper- én begeleidercode allemaal naar dezelfde `mna_chat`-thread zonder rolcheck; POST accepteert bovendien een vrij `auteur`-veld uit de request-body zonder verificatie — een koper kan een bericht sturen dat bij de verkoper verschijnt als afkomstig van de eigen adviseur, inclusief de automatische meldingsmail. Exact de dataklasse die `SECURITY-INVARIANTS.md` nooit naar de koper wil laten gaan, en exact het bugpatroon uit werkregel 11d ("dit platform had al meerdere kritieke cross-rol-datalekken"). Locatie: `backend/worker/17-mna-chat.js:6-31`.
  2. **CI staat al 50+ opeenvolgende runs (minstens 5 dagen) rood op de zwaarste teststap, onopgemerkt.** Niet de eerder gefixte YAML-fout (die liet niets draaien) — 5 van de 6 stappen slagen wél, maar "volledige e2e-testsuite tegen staging" (adviseur-lifecycle, traject-flow, module-gating, AVG-verwijdering) faalt structureel. Waarschijnlijke link met de hieronder genoemde Playwright-flakiness. Root cause nog niet vastgesteld (logs vereisen een GitHub-token, niet beschikbaar binnen de subagent-sessie).

  **5 P2's:** sequentiële per-rij INSERT's i.p.v. `env.DB.batch()` bij bankmutatie-CSV-import (schaalrisico bij een realistische upload); ~200+ formuliervelden met een niet-programmatisch-gekoppeld label (label/`for` ontbreekt) op de drie hoofdportalen; het adviseursportaal se "Terug naar overzicht" alleen met de muis bedienbaar (navigatie-blokkade); toast-meldingen missen `aria-live`; geen rate-limiting op chat-/Q&A-schrijfroutes (spam-/kostenmisbruik-vector).

  **6 P3's + 6 P4's** — zie artifact voor de volledige lijst; noemenswaardig: een misleidend "wordt door niets aangeroepen"-commentaar in de security-kritieke `worker/00-policy.js` (regel 6, terwijl deze module de daadwerkelijke implementatie achter `begeleiderAuth`/`rolVanCode` is); de vandaag doorgevoerde backup-fix (CLOUDFLARE_API_TOKEN) staat nog geen dag in de launchd-plist en is dus nog niet door een echte geplande run bewezen; groeiende monolithische bestanden (verder toegenomen, ook al binnen dezelfde dag); CLAUDE.md's moduleaantal is stale (28 i.p.v. 20, met een naam-botsing op "21-").
