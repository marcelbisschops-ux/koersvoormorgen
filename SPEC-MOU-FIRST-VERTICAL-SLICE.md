...
| **Review engine** | `BlockReview` per `block_id@version@dossier`, statussen `REQUIRED → REQUESTED → IN_REVIEW → CHANGES_REQUESTED → APPROVED → REVOKED → SUPERSEDED`. Interim reviewer = `AdvisorReviewer` (de begeleider bevestigt met naam + datum, gelogd); dezelfde interface waar straks `LegalSpecialist` in past. **Zelf-review** (opsteller = reviewer) wordt expliciet gelogd als `review_kind = SELF_REVIEW`; heeft een traject meer dan één adviseur, dan moet de interim-reviewer een ander zijn dan de opsteller. De slice bewijst de *mechaniek* (statussen, invalidatie, gates), niet de *governance-waarde* — die komt met de echte specialistrollen (FASE E). |
