...
6. ~~Cross-path-informatielek-audit F1-F13~~ — **11 van 13 afgerond 19 augustus 2026**: cross-traject-auth-lekken (F1/F2), koper-categorie-intrekking niet doorgevoerd naar 4 routefamilies (F3), marcel-CC onvoorwaardelijk bij externe-adviseurstrajecten in 6+ mailflows (F4), chat-state niet gereset bij uitloggen (F5), waarderingsgeschiedenis zonder auth (F6), zwakke validatie op mail-begeleider (F7), muur tegen externe adviseurs miste 4 gesprek-bijlage-routes (F8), existence-oracle bij Q&A-reactie (F10), gesprek-bijlage-`traject_id` nooit gevuld → wees-R2-bestanden bij trajectverwijdering (F11), meekijkers-route zonder muur bij admin-key-aanroep (F13). Volledige testsuite + nieuwe permanente regressietest `tests/e2e-crosspath-fixes.mjs` (39/39) groen tegen productie, beide repo's gepusht. Volledig logboek: `CROSS-PATH-SECURITY-STANDAARD.md`. **Open, vereist Marcels beslissing (niet zelfstandig ingevuld):**
   - **F9** — dode `/mna/groep/*`-routes (`worker/07-mna-groepen.js`) zonder eigen autorisatie, bevestigd ongebruikt door de huidige frontend. Verwijderen, of alsnog autoriseren voor het geval de feature ooit geactiveerd wordt?
   - **F12** — de generieke `/ai`-proxy (`worker/06-scantool.js`) heeft geen server-side traject-binding. Architectuurrichting voor nieuwe AI-features (dedicated, code-geauthenticeerde endpoints) is als richtlijn vastgelegd, maar een retroactieve fix aan bestaand `/ai`-gebruik is een aparte scope-beslissing.
