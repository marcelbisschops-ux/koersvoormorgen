...
|---|------|--------------------|-------------------------------|
| 16 | Foreign keys toevoegen aan het D1-schema | **Plan klaar (26 juli 2026), nog niet uitgevoerd** — zie `MIGRATIE-FOREIGN-KEYS.md` in de backend-repo. Bevestigd: D1 dwingt FK's direct af (anders dan vanilla SQLite), en er bestaan nu al 54 wees-rijen over 4 tabellen die eerst inhoudelijk uitgezocht moeten worden vóór een migratie kan starten — één ervan (`FASE246082` als traject_id) ziet er niet uit als een normale trajectcode. `schema.sql` in deze repo is bovendien een verouderd artefact dat wél FK's claimt — niet actief gebruikt, wel verwarrend | Stap 0 uit het plan afronden (wees-rijen uitzoeken/opschonen, staging vullen met een productiekopie) + jouw akkoord op het plan, dan pas uitvoeren |
