...
4. **OPENSTAAND, PROMINENT (Marcel, 19 augustus 2026) — volledig MKB-overname-testscript.** Marcel wil zelf een complete, realistische overname end-to-end kunnen simuleren (verkoper-upload → DD → waardering → dealvoorstel → NDA/LoI/BEM/Exclusiviteit → closing) om functionaliteit én veiligheid zelf te kunnen controleren, niet alleen op Claudes eigen audits vertrouwen. Nog te bouwen: een volledig testdocumentenpakket volgens de bestaande **Testdocumenten-standaard** in `CLAUDE.md` (alle DD-fases van het sectorprofiel, realistische vorm/lengte, gemengde bestandsformaten, minstens één randgeval dat hoort te worden afgewezen, holdingstructuur) + een stapsgewijs testscript dat Marcel zelf kan doorlopen. Directe aanleiding: sessie van 19 augustus 2026 waarin bleek dat gegenereerde NDA/LoI-documenten die nooit verstuurd/getekend werden, spoorloos verdwenen (nu gefixt, zie git-log) — dit soort gaten had een end-to-end testscript eerder blootgelegd.
