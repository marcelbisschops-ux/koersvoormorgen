...
| 17 | Volledige modal-toegankelijkheid (aria/role/toetsenbord) | De P2-fix van 25 juli dekte alleen de Dealvoorstel-modal. 9 van 10 onderzochte modals (koper-toegang, groepsstructuur, partners, de 5 adv.html-modals) missen nog `role="dialog"`/`aria-modal`/`label-for`. **Leesbaarheid in dark mode (het andere deel van dit punt) is 26 juli 2026 al gefixt** — zie logboek hieronder; dit punt gaat nu alleen nog over toetsenbord/screenreader-toegankelijkheid | Eén gerichte ronde: een generieke `maakModal()`-helper voor alle 14+ modals tegelijk, i.p.v. losse patches |
| 18 | HTTP-statuscode-consistentie | 104 van de responses missen een expliciete status (blijven impliciet 200) — bijv. hetzelfde "DB niet beschikbaar" geeft op de ene plek 503, op de andere 200. **Bevestigd risico (26 juli 2026):** frontend-code checkt op meerdere plekken écht `resp.ok`/`resp.status` (niet alleen `d.error`), bijv. `mna/02-state-opslag-documenten.js:1248`, `mna/06-schermen.js:757/827` — een statuscode-wijziging kán dus per-endpoint het foutafhandelingspad daadwerkelijk raken, dit is geen puur cosmetische opschoning | Een gerichte ronde die alle ~104 nazoekt, per endpoint checkt of frontend op status i.p.v. alleen `.error` let, en normaliseert — niet blind een paar losse endpoints |
