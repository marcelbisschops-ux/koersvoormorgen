...
| 7 | Branding neutraliseren (white-label basis) — 🟡 FUNDAMENT AF | Sonnet | 6–10 u | €600–1.000 | Centrale `BRAND`-config in mna.html (productnaam/exploitant/suffix); alle UI-chrome (9 headers, paginatitel, footer) eraan gekoppeld — één wijziging = overal. Live bewezen (→ "DealScope"). **adv.html + VerhuisScan (hugo.html & verhuis.html) óók af** (BRAND-config, live bewezen → merknaam wisselt overal). Rest: index.html/marilyn.html + dieper: legal-templates & AI-prompts die "Bisschops Financing"/"accountancy" noemen |
