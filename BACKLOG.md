...
- **Foreign keys D1-schema (voorheen punt 16) — VOLLEDIG AFGEROND**: alle 19 tabellen die naar `mna_trajecten` verwijzen hebben nu `FOREIGN KEY (traject_id) REFERENCES mna_trajecten(id) ON DELETE CASCADE`. Eerst een pilot (2 lege tabellen), daarna op Marcels expliciete akkoord dezelfde dag de resterende 16 in één batch. Onderweg ontdekt: `mna_documenten` had al een werkende FK (scheelde 1 migratie); staging en productie bleken structureel uit elkaar gegroeid (schema-drift, o.a. ontbrekende tabellen/kolommen op staging) — het migratiescript las daarom het schema per tabel live in i.p.v. een vaste kolomlijst aan te nemen. Rijentelling exact gelijk vóór/na bij elke tabel, cascade-delete + FK-verwerping getest, volledige testsuite groen. Zie `MIGRATIE-FOREIGN-KEYS.md` in de backend-repo. **Nog bewust buiten scope**: `mna_vok` (verwijst via `tussen_code`) en `mna_audit` (verwijst via 3 mogelijke kolommen) — passen niet in het standaardpatroon, vragen een eigen aanpak. Backup-tabellen (19×, `*_backup_20260726`) minstens 14 dagen laten staan.
