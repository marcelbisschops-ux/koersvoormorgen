    ...
    document.getElementById('dv-ok').onclick=async function(){
      ...
      try{
        ...
        var contextBlok='Sector: '+(sectorProfiel.label||'MKB')+'. Verkopende partij: '+(t2.kantoor_naam||S.code)+'. Kopende partij: '+p.koperNaam+'.\n'
          ...
          +('\nOnderhandelruimte / trade-space (herindeling van de opbrengst-brug naar zekerheid × timing, geen nieuwe aannames): totale tegenprestatie voor de verkoper €'+Math.round(zopa.totaal)+' — zeker bij closing €'+Math.round(zopa.zekerNu)+' ('+zopa.pctZekerBijClosing.toFixed(0)+'%), escrow €'+Math.round(zopa.escrow)+' ('+zopa.pctEscrow.toFixed(0)+'%), voorwaardelijk/uitgesteld €'+Math.round(zopa.voorwaardelijk)+' ('+zopa.pctVoorwaardelijk.toFixed(0)+'%'+((zopa.earnOut>0||zopa.vendorLoan>0)?': '+[zopa.earnOut>0?'earn-out':'',zopa.vendorLoan>0?'verkoperslening':''].filter(Boolean).join(' + '):'')+'), behouden belang €'+Math.round(zopa.behoudenBelangWaarde)+' ('+zopa.pctBehoudenBelang.toFixed(0)+'%). Dit toont waar de verkoper aan kan draaien in de onderhandeling.')
        ...
        var koppen='## Managementsamenvatting\n(3-5 bullets over de kern van het voorstel, dan één alinea "In één zin")\n\n'
          ...
          +'## Waardering: marktonderbouwing\n(presenteer de multiple-range expliciet als het gekozen uitgangspunt van de begeleider, met de sectornormen als rationale erbij — bijv. "Gekozen uitgangspunt: X–Yx. Rationale: [sectornormen/omvang/marge]." Niet formuleren alsof de range zelf een objectief gegeven/vaststaande sectorpraktijk is — het is een waarderingskeuze die de sectornormen als onderbouwing gebruikt, geen automatische uitkomst daarvan)\n\n'
          ...
          +'## Onderhandelruimte: verdeling van de tegenprestatie\n(interpreteer de verdeling hieronder in 3-4 zinnen vanuit het perspectief van de verkoper: welk deel is zeker bij closing, welk deel is uitgesteld maar doorgaans zeker (escrow), welk deel is écht voorwaardelijk (earn-out, eventuele verkoperslening) en welk deel is behouden belang waarvan de waarde pas bij een tweede exit vaststaat. Benoem concreet waar de verkoper in de onderhandeling aan kan draaien — bijv. een groter deel cash bij closing tegen een lagere prijs, een kortere escrow, een kleinere earn-out-tranche, of meer/minder behouden belang. BELANGRIJK: dit is een herindeling van de opbrengst-brug hierboven, geen nieuwe berekening of aanname; "zeker" betekent contant op de closingdatum, niet gegarandeerd. Gebruik uitsluitend de tabel, verzin geen bedragen of percentages)\n[TABEL:ZOPA]\n\n'
