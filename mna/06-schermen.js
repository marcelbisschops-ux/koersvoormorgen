function renderCover(){
  ...
  if(isVerkoper()){
    intro='<p>Geachte '+esc(t.contact_naam||'relatie')+',</p>'
      ...
      +'<div style="margin:1rem 0;padding:.75rem 1rem;background:var(--card);border:1px solid var(--border);border-radius:var(--r);font-size:11px;color:var(--muted);line-height:1.7">&#128274; <strong>Beveiliging &amp; AVG:</strong> Alle gegevens worden versleuteld via HTTPS verstuurd en opgeslagen op Cloudflare-servers in Europa (Frankfurt, EU) en uitsluitend gebruikt voor dit M&amp;A-traject. <strong>'+esc(t.begeleider_naam||'Uw adviseur')+'</strong> is de verwerkingsverantwoordelijke voor uw gegevens conform de AVG — voor vragen over inzage, correctie of verwijdering van úw gegevens kunt u het beste rechtstreeks contact opnemen'+(t.begeleider_email?' via <a href=\"mailto:'+esc(t.begeleider_email)+'\" style=\"color:var(--teal)\">'+esc(t.begeleider_email)+'</a>':'')+'. '+BRAND.kort+' verzorgt als technisch verwerker de beveiliging van het platform; zie de <a href=\"privacy.html\" style=\"color:var(--teal)\">privacyverklaring van '+BRAND.kort+'</a> voor die technische verwerking.</div>'
    ...
    intro='<p>Geachte relatie,</p>'
      ...
      +'<div style="margin:1rem 0;padding:.75rem 1rem;background:var(--card);border:1px solid var(--border);border-radius:var(--r);font-size:11px;color:var(--muted);line-height:1.7">&#128274; <strong>Beveiliging &amp; AVG:</strong> <strong>'+esc(t.begeleider_naam||'Uw adviseur')+'</strong> is de verwerkingsverantwoordelijke voor de gegevens in dit traject conform de AVG'+(t.begeleider_email?' — voor vragen kunt u contact opnemen via <a href=\"mailto:'+esc(t.begeleider_email)+'\" style=\"color:var(--teal)\">'+esc(t.begeleider_email)+'</a>':'')+'. '+BRAND.kort+' verzorgt als technisch verwerker de beveiliging van het platform; zie de <a href=\"privacy.html\" style=\"color:var(--teal)\">privacyverklaring van '+BRAND.kort+'</a>.</div>'
      +'<p>Met vriendelijke groet,<br><strong>' + BRAND.contactpersoon + '</strong><br><span style="font-size:12px;color:var(--muted)">Senior M&amp;A-adviseur &middot; ' + BRAND.bedrijfKort + '</span></p>';
  }
  ...
  if(docKnopBtn&&isTussen()){
    docKnopBtn.onclick=function(){
      ...
      async function bgGenereerDoc(type){
        ...
        var tplR=await fetch(WORKER+'/mna/template/'+tplType+'?email='+encodeURIComponent(t3.begeleider_email||'')).catch(function(){return{json:function(){return{ok:false};}};});
