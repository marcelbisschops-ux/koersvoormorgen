  ...
  try {
    ...
    var lijst = d.versies.slice().reverse().map(function(v) {
      ...
      return '<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--card);border:1px solid var(--border);border-radius:var(--r)">'
        ...
        + '<span style="font-size:12px;flex:1">'+(v.bewaard?'<a href="'+WORKER+'/mna/document/download/'+v.id+'?code='+encodeURIComponent(S.code)+'" target="_blank" rel="noopener" style="color:var(--teal)">'+esc(v.bestand_naam)+'</a>':esc(v.bestand_naam))+'</span>'
      ...
      fd.forEach(function(doc){
        ...
        html+='<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border)">'
          ...
          +(toonKoppelen?'<div style="margin-top:6px;display:flex;gap:6px;align-items:center">'
            ...
            +'</div>':'')
          +'</div>'
          +'<a href="'+WORKER+'/mna/document/download/'+doc.id+'?code='+encodeURIComponent(S.code)+'" target="_blank" rel="noopener" class="btn-ghost btn-sm" style="font-size:11px;text-decoration:none" onclick="secAuditLog(\'document_bekeken\',{doc_naam:\''+doc.naam.replace(/'/g,'')+'\'})">&#8681; '+(isKoper()?'Bekijken':'Download')+'</a>'
          +'</div>';
      });
      html+='</div>';
    });
