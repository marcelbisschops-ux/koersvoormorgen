...
> **Update 2026-09-03 — FASE 5 voorbereid.** Marcel heeft de commerciële keuzes gemaakt (ISSUE-05: cap € 10.000/opdracht + € 10.000/jaar, geen verzekering · ISSUE-03: voorwaardelijke consumentenclausule · ISSUE-07: strakke Transactiewaarde-definitie · ISSUE-09: verrekenbare, begrensde boete · ISSUE-02: geen losse AV-pagina · ISSUE-12: AV-bepaling + adviseur-beslisboom, niet-blokkerend). De volledige nieuwe tekst per document staat in [`FASE5-WIJZIGINGEN.md`](FASE5-WIJZIGINGEN.md); de Wwft-werkinstructie in [`CLIENTACCEPTATIE-BESLISBOOM.md`](CLIENTACCEPTATIE-BESLISBOOM.md). Nog toe te passen op de live bestanden na "ja op de redline". Eén open sub-keuze: ISSUE-10 H1 (voorstel A met € 10.000-bodem, of voorstel B).  → **beslist:** vast € 10.000/traject + samenloopregel (live).
> **Update 2026-09-03 (2e ronde) — verder toegepast:** marilyn `mna_templates` bem_verk/bem_koper verwijderd (backup op Bureaublad) → alle 3 BEM's gebruiken nu de gefixte code-standaard. VOK single-source (`bouwVokTekst()` + `GET /mna/vok/tekst`) + ondertekende tekst opgeslagen in `mna_vok.tekst` (ISSUE-01 E2, live). `bgDoc()` placeholder-waarschuwing + confirm-gate (ISSUE-25, live). Matching: `akkoord`-verklaring verplicht bij koper-zelfregistratie (ISSUE-21, live). Meekijker (ISSUE-22): bleek al afgedekt (verplichte vertrouwelijkheidsverklaring vóór inzage) — fase-scope + GV-link toegevoegd, verder geen wijziging nodig. Wwft (ISSUE-12/JUR-1): analyse in [`WWFT-ANALYSE.md`](WWFT-ANALYSE.md) — voorlopige conclusie: meer waarschijnlijk wél Wwft-plichtig; specialistische toets is actie voor Marcel. **Nog te bouwen:** cliëntacceptatie-checkbox in het begeleider-dashboard (ISSUE-12 actie C) + Google Fonts self-hosten (ISSUE-11).
